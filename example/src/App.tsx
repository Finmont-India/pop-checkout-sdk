...
const App = () => {
  ...
  const { getInfo, Widget, authorizeCard, initializePayment, getClaro } = useSdk();
  ...
  const fetchData = async () => {
    try {
      ...
      console.error("Error in fetching instance:", error);
    }
  };
  ...
  }, [getInfo, dataPayload, getClaro,initializePayment]);
  ...
  return (
    <div className='h-vh w-vw'>
      ...
      {/* <NethoneProfiling
        ...
        /> */}
         <div>
        <button onClick={fetchData}>claropay</button>
      </div>
    </div>
    </div>
  );
